...
You can use this library in your Unity game by using the Package Manager and referencing the package `com.Fixed.Mathematics`. See the forum [Welcome](https://forum.unity.com/threads/welcome.522627) page for more details.
...
Open the `src\Fixed.Mathematics.sln` under Visual Studio 2015 or MonoDevelop and compile in Debug\Release.
...
To signify that these types are bultin their type names are in all lower case. The operators on these bultin types found in `Fixed.Mathematics.math` are considered intrinsics and are thus always in lower case.
