...
//			pixel_xy /= 2;
			...
			int2 firstCell = positionBuffer[i.instanceID].firstCell;
            ...
            uint dword = GetUint(_FontTex, _FontTex_TexelSize, encoded_xy.x, encoded_xy.y);

            uint bitInDword = bitInCell & 31;
            uint color = (dword >> bitInDword) & 1;
			if(color)
			{
			    uint bg = (cell.color >> 4) & 0x07;
//			    if(bg == 0)
//			        discard;
				return Palette(bg);
			}
