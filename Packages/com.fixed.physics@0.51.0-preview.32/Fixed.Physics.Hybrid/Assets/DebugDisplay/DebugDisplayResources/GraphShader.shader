...
//			p.y = 1.0 - p.y;
		...
		fixed4 frag(v2f i) : SV_Target
		{
            ...
            xy.y = _ScreenParams.y-1 - xy.y; // darn flipped

            xy -= instanceBuffer[instID].screenPosition;
			...
			int2 subsamples = int2(7,7);
			int totalSubsamples = subsamples.x * subsamples.y;
			float2 step = float2(1.0 / subsamples.x, 1.0 / subsamples.y);
