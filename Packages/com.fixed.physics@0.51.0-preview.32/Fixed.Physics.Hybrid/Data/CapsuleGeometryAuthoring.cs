...
namespace Fixed.Physics.Authoring
{
        ...
        public override int GetHashCode()
        {
            return unchecked((int)math.hash(
                new float3x3(
                    ...
                    new float3((sfloat)(int)m_OrientationEuler.RotationOrder, m_Height, m_Radius)
                )
            ));
        }
    }
    ...
    public static class CapsuleGeometryAuthoringExtensions
    {
        ...
        {
            var halfHeight   = (sfloat)0.5f * input.Height;
            var halfDistance = halfHeight - input.Radius;
            var axis         = math.normalize(math.mul(input.Orientation, new float3 { z = sfloat.One }));
