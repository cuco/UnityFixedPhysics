...
namespace Fixed.Physics
{
    ...
    public struct CollisionEvent : ISimulationEvent<CollisionEvent>
    {
        ...
        public fp3 Normal => EventData.Value.Normal;
        ...
        public struct Details
        {
            ...
            public NativeArray<fp3> EstimatedContactPointPositions;

            // Estimated total impulse applied
            public fp EstimatedImpulse;

            // Calculate the average contact point position
            public fp3 AverageContactPointPosition
            {
                get
                {
                    var pos = fp3.zero;
    ...
    public struct CollisionEvents /* : IEnumerable<CollisionEvent> */
    {
        ...
        private readonly fp m_TimeStep;

        internal CollisionEvents(NativeStream eventDataStream, NativeArray<Velocity> inputVelocities, fp timeStep)
        {
        ...
        public struct Enumerator /* : IEnumerator<CollisionEvent> */
        {
            ...
            internal Enumerator(NativeStream stream, NativeArray<Velocity> inputVelocities, fp timeStep)
            {
    ...
    struct CollisionEventData
    {
        ...
        internal fp SolverImpulse;

        // Number of narrow phase contact points
        internal int NumNarrowPhaseContactPoints;

        internal unsafe CollisionEvent CreateCollisionEvent(fp timeStep, NativeArray<Velocity> inputVelocities)
        {
        ...
        internal unsafe CollisionEvent.Details CalculateDetails(
            ref PhysicsWorld physicsWorld, fp timeStep, Velocity inputVelocityA, Velocity inputVelocityB, NativeArray<ContactPoint> narrowPhaseContactPoints)
        {
            int bodyIndexA = BodyIndices.BodyIndexA;
            int bodyIndexB = BodyIndices.BodyIndexB;
            ...
            fp estimatedImpulse = SolverImpulse;

            // First calculate minimum time of impact and estimate the impulse
            fp toi = timeStep;
            {
                fp sumRemainingVelocities = fp.zero;
                fp numRemainingVelocities = fp.zero;
                for (int i = 0; i < narrowPhaseContactPoints.Length; i++)
                {
                    ...
                    {
                        fp3 pointVelA = GetPointVelocity(motionDataA.WorldFromMotion,
                            motionVelocityA.LinearVelocity, motionVelocityA.AngularVelocity, cp.Position + Normal * cp.Distance);
                        fp3 pointVelB = GetPointVelocity(motionDataB.WorldFromMotion,
                            motionVelocityB.LinearVelocity, motionVelocityB.AngularVelocity, cp.Position);
                        fp projRelVel = fpmath.dot(pointVelB - pointVelA, Normal);
                        if (projRelVel > fp.zero)
                        {
                            sumRemainingVelocities += projRelVel;
                            numRemainingVelocities += fp.one;
                        }
                    }
                    ...
                    // Get minimum time of impact
                    {
                        ...
                        if (projRelVel > fp.zero)
                        {
                            fp newToi = fpmath.max(fp.zero, cp.Distance / projRelVel);
                            toi = fpmath.min(toi, newToi);
                        }
                        else if (cp.Distance <= fp.zero)
                        {
                ...
                if (numRemainingVelocities > fp.zero)
                {
                    fp sumInvMass = motionVelocityA.InverseMass + motionVelocityB.InverseMass;
            ...
            fp minDistance = fp.max_value;
            {
                ...
                for (int i = 0; i < narrowPhaseContactPoints.Length; i++)
                {
                    ...
                    var cp = narrowPhaseContactPoints[i];
                    {
                        fp3 pointVelA = GetPointVelocity(motionDataA.WorldFromMotion, inputVelocityA.Linear, inputVelocityA.Angular, cp.Position + Normal * cp.Distance);
                        fp3 pointVelB = GetPointVelocity(motionDataB.WorldFromMotion, inputVelocityB.Linear, inputVelocityB.Angular, cp.Position);
                        fp3 relVel = pointVelB - pointVelA;
                        fp projRelVel = fpmath.dot(relVel, Normal);
                ...
                {
                    EstimatedContactPointPositions = new NativeArray<fp3>(estimatedContactPointCount, Allocator.Temp),
        ...
        private static fp3 GetPointVelocity(FpRigidTransform worldFromMotion, fp3 linVel, fp3 angVel, fp3 point)
        {
            fp3 angularVelocity = fpmath.rotate(worldFromMotion, angVel);
            fp3 linearVelocity = fpmath.cross(angularVelocity, point - worldFromMotion.pos);
