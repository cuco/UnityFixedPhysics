...
namespace Fixed.Physics.Tests.Collision.Queries
{
    class QueryTests
    {
        ...
        static void ValidateDistanceResult(DistanceQueries.Result result, ref ConvexHull a, ref ConvexHull b, MTransform aFromB, fp referenceDistance, string failureMessage)
        {
            ...
            Assert.AreEqual((float)result.Distance, (float)referenceDistance, (float)adjustedTolerance, failureMessage + ": incorrect distance");
        ...
        static unsafe void CheckRaycastHit(ref Physics.PhysicsWorld world, RaycastInput input, RaycastHit hit, string failureMessage)
        {
            ...
            {
                Assert.AreEqual((float)distanceHit.Distance, 0.0f, (float)tolerance, failureMessage);
            }
        }
