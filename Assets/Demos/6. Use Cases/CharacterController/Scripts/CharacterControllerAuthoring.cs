...
public struct CharacterControllerComponentData : IComponentData
{
    ...
    public sfloat MovementSpeed;
    public sfloat MaxMovementSpeed;
    public sfloat RotationSpeed;
    public sfloat JumpUpwardsSpeed;
    public sfloat MaxSlope; // radians
    public int MaxIterations;
    public sfloat CharacterMass;
...
{
    public sfloat CurrentRotationAngle;
...
{
    static readonly sfloat k_DefaultTau = (sfloat)0.4f;
    static readonly sfloat k_DefaultDamping = (sfloat)0.9f;
    ...
    struct CharacterControllerJob : IJobEntityBatch
    {
        ...
        public unsafe void Execute(ArchetypeChunk batchInChunk, int batchIndex)
        {
            ...
            for (int i = 0; i < batchInChunk.Count; i++)
            {
                ...
                var up = math.select(math.up(), -math.normalize(ccComponentData.Gravity),
                    math.lengthsq(ccComponentData.Gravity) > sfloat.Zero);
            ...
            ref CharacterControllerInternalData ccInternalData, ref float3 linearVelocity)
        {
            ...
            float3 requestedMovementDirection = float3.zero;
            {
                ...
                sfloat horizontal = ccInternalData.Input.Movement.x;
                sfloat vertical = ccInternalData.Input.Movement.y;
                bool jumpRequested = ccInternalData.Input.Jumped != 0;
                ccInternalData.Input.Jumped = 0; // "consume" the event
                bool haveInput = (math.abs(horizontal) > sfloat.Epsilon) || (math.abs(vertical) > sfloat.Epsilon);
                if (haveInput)
                {
            ...
            // Turning
            {
                sfloat horizontal = ccInternalData.Input.Looking.x;
                bool haveInput = (math.abs(horizontal) > sfloat.Epsilon);
                if (haveInput)
                {
                ...
                {
                    ccInternalData.Velocity.Angular = sfloat.Zero;
                }
            }
        ...
        private void CalculateMovement(sfloat currentRotationAngle, float3 up, bool isJumping,
            float3 currentVelocity, float3 desiredVelocity, float3 surfaceNormal, float3 surfaceVelocity, out float3 linearVelocity)
        {
            ...
            float3 diff;
            {
                ...
                sfloat fwd = math.dot(desiredVelocity, forward);
                sfloat side = math.dot(desiredVelocity, sideVec);
                sfloat len = math.length(desiredVelocity);
                float3 desiredVelocitySF = new float3(-side, -fwd, sfloat.Zero);
    ...
    protected override void OnUpdate()
    {
        ...
        var ccJob = new CharacterControllerJob
        {
            ...
            // Input
            DeltaTime = (sfloat)Time.DeltaTime,
