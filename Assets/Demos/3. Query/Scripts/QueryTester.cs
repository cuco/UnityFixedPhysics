...
using static Fixed.Physics.CompoundCollider;
...
namespace Fixed.Physics.Extensions
{
    ...
    public unsafe class QueryTester : MonoBehaviour
    {
        ...
        {
            new float3((sfloat)0.5f, (sfloat)0.5f, sfloat.Zero),
            new float3((sfloat)0.5f, -(sfloat)0.5f, sfloat.Zero),
            new float3(-(sfloat)0.5f, -(sfloat)0.5f, sfloat.Zero),
            new float3(-(sfloat)0.5f, (sfloat)0.5f, sfloat.Zero)
        };
        ...
        private BlobAssetReference<Collider> CreateCollider(ColliderType type)
        {
            ...
            switch (type)
            {
                ...
                case ColliderType.Capsule:
                    ...
                    {
                        Vertex0 = new float3(sfloat.Zero, -sfloat.Half, sfloat.Zero),
                        Vertex1 = new float3(sfloat.Zero, sfloat.Half, sfloat.Zero),
                        Radius = sfloat.Half
                    });
                    break;
                ...
                case ColliderType.Compound:
                    ...
                    childrenBlobs[0] = new ColliderBlobInstance
                    {
                        ...
                        {
                            pos = new float3(sfloat.Half, sfloat.Zero, sfloat.Zero),
                ...
                case ColliderType.Terrain:
                    ...
                    var heights = new NativeArray<sfloat>(numSamples, Allocator.TempJob, NativeArrayOptions.UninitializedMemory);
                    for (int i = 0; i < numSamples; i++)
                    {
                        heights[i] = rand.NextFloat(sfloat.Zero, sfloat.One);
                    }
        ...
        void RunQueries()
        {
            ...
            float3 direction = (float3)(transform.rotation * Direction) * Distance;
        ...
        public sfloat Distance = (sfloat)10.0f;
        ...
        void OnDrawGizmos()
        {
            if (Simulating)
            {
                ...
                if (RaycastHits.IsCreated)
                {
                    foreach (RaycastHit hit in RaycastHits.ToArray())
                    {
                        ...
                        Assert.IsTrue(math.abs(math.lengthsq(hit.SurfaceNormal) - sfloat.One) < (sfloat)0.01f);
            ...
            else
            {
                ...
                Vector3 dir = (t.rotation * Direction) * (float)Distance;
