...
namespace Fixed.Physics.Editor
{
    ...
    public class EditorUtilities
    {
        ...
        public class AxisEditor
        {
            ...
            {
                sfloat lengthSq = math.lengthsq(x);
                const float epsSq = 1e-8f;
                if ((float)math.abs(lengthSq - sfloat.One) > epsSq)
                {
                    if ((float)lengthSq > epsSq)
                    {
            ...
            private static bool NormalizePerpendicular(float3 axis, ref float3 perpendicular)
            {
                ...
                sfloat dot = math.dot(axis, perpendicular);
                sfloat absDot = math.abs(dot);
                if (absDot > sfloat.One - (sfloat)1e-5f)
                {
        ...
        public static void EditLimits(RigidTransform worldFromA, RigidTransform worldFromB, float3 pivotA, float3 axisA, float3 axisB, float3 perpendicularA, float3 perpendicularB,
            ref sfloat minLimit, ref sfloat maxLimit, JointAngularLimitHandle limitHandle, Object target)
        {
            ...
            sfloat angle = CalculateTwistAngle(new quaternion(jointBFromA), 0); // index = 0 because axis is the first column in worldFromJoint
            ...
            limitHandle.xMin = -(float)maxLimit;
            limitHandle.xMax = -(float)minLimit;
            ...
            using (new Handles.DrawingScope(handleMatrix))
            {
                ...
                if (EditorGUI.EndChangeCheck())
                {
                    ...
                    minLimit = -(sfloat)limitHandle.xMax;
                    maxLimit = -(sfloat)limitHandle.xMin;
                }
            }
        }
    }
