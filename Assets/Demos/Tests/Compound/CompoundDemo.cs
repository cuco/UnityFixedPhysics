...
public class CompoundDemoSystem : SceneCreationSystem<CompoundDemoScene>
{
    public override void CreateScene(CompoundDemoScene sceneSettings)
    {
        //         // Floor
        //         {
        //             BlobAssetReference<Fixed.Physics.Collider> collider = Fixed.Physics.BoxCollider.Create(new float3(0, -0.1f, 0), Quaternion.identity, new float3(10.0f, 0.1f, 10.0f), 0.05f);
        ...
        // Dynamic compound
        {
            ...
            var children = new NativeArray<CompoundCollider.ColliderBlobInstance>(3, Allocator.Temp)
            {
                [0] = new CompoundCollider.ColliderBlobInstance
                {
                    ...
                    Collider = Fixed.Physics.BoxCollider.Create(box)
                },
                [1] = new CompoundCollider.ColliderBlobInstance
                {
                    CompoundFromChild = RigidTransform.identity,
                    Collider = Fixed.Physics.SphereCollider.Create(sphere)
                },
            ...
            BlobAssetReference<Fixed.Physics.Collider> collider = CompoundCollider.Create(children);
            CreatedColliders.Add(collider);
            children.Dispose();

            CreateDynamicBody(new float3(0, 1, 0), quaternion.identity, collider, float3.zero, float3.zero, (sfloat)1.0f);
        }
    }
}
