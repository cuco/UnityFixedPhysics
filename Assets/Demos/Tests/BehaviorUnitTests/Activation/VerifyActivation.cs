...
namespace Fixed.Physics.Tests
{
    ...
    public partial class VerifyActivationSystem : SceneCreationSystem<VerifyActivationScene>
    {
        ...
        {
            // Common params
            float3 groundSize = new float3((sfloat)5.0f, (sfloat)1.0f, (sfloat)5.0f);
            float3 boxSize = new float3((sfloat)1.0f, (sfloat)1.0f, (sfloat)1.0f);
            sfloat mass = (sfloat)1.0f;
            ...
            if (data.PureFilter == 1)
            {
                ...
                CreateStaticBody(new float3(-(sfloat)30.0f, (sfloat)0.0f, (sfloat)0.0f), quaternion.identity, groundCollider);
                var boxCollider = BoxCollider.Create(new BoxGeometry { Orientation = quaternion.identity, Size = boxSize });
                CreateDynamicBody(new float3(-(sfloat)30.0f, (sfloat)1.0f, (sfloat)0.0f), quaternion.identity, boxCollider, float3.zero, float3.zero, mass);
            ...
            {
                var groundCollider = BoxCollider.Create(new BoxGeometry { Orientation = quaternion.identity, Size = groundSize });
                CreateStaticBody(new float3(-(sfloat)20.0f, (sfloat)0.0f, (sfloat)0.0f), quaternion.identity, groundCollider);
                var boxCollider = BoxCollider.Create(new BoxGeometry { Orientation = quaternion.identity, Size = boxSize });
                CreateDynamicBody(new float3(-(sfloat)20.0f, (sfloat)1.0f, (sfloat)0.0f), quaternion.identity, boxCollider, float3.zero, float3.zero, mass);
            ...
            {
                var groundCollider = BoxCollider.Create(new BoxGeometry { Orientation = quaternion.identity, Size = groundSize });
                CreateStaticBody(new float3(-(sfloat)10.0f, (sfloat)0.0f, (sfloat)0.0f), quaternion.identity, groundCollider);
                var boxCollider = BoxCollider.Create(new BoxGeometry { Orientation = quaternion.identity, Size = boxSize });
                CreateDynamicBody(new float3(-(sfloat)10.0f, (sfloat)1.0f, (sfloat)0.0f), quaternion.identity, boxCollider, float3.zero, float3.zero, mass);
            ...
            {
                var groundCollider = BoxCollider.Create(new BoxGeometry { Orientation = quaternion.identity, Size = groundSize });
                CreateStaticBody(new float3((sfloat)0.0f, (sfloat)0.0f, (sfloat)0.0f), quaternion.identity, groundCollider);
                var boxCollider = BoxCollider.Create(new BoxGeometry { Orientation = quaternion.identity, Size = boxSize });
                CreateDynamicBody(new float3((sfloat)0.0f, (sfloat)1.0f, (sfloat)0.0f), quaternion.identity, boxCollider, float3.zero, float3.zero, mass);
        ...
        protected override void OnUpdate()
        {
            ...
            if (m_Counter == 30)
            {
                ...
                if (verificationComponentData.MotionChange > 0)
                {
                    ...
                    EntityManager.AddComponentData(staticEntities[counter], PhysicsMass.CreateDynamic(colliderComponent.MassProperties, (sfloat)1.0f));
                    EntityManager.AddComponentData(staticEntities[counter], new PhysicsVelocity
                    {
                        Linear = new float3((sfloat)0.0f, -(sfloat)1.0f, (sfloat)0.0f),
                        Angular = float3.zero
                    });
                    counter++;
                }
                ...
                if (verificationComponentData.Teleport > 0)
                {
                    ...
                    translationComponent.Value.y = -(sfloat)10.0f;
                ...
                if (verificationComponentData.ColliderChange > 0)
                {
                    ...
                    colliderComponent.Value = BoxCollider.Create(new BoxGeometry { Orientation = quaternion.identity, Size = new float3((sfloat)5.0f, (sfloat)1.0f, (sfloat)50.0f) });
                ...
                {
                    var colliderComponent = EntityManager.GetComponentData<PhysicsCollider>(staticEntities[counter]);
                    var newColliderComponent = BoxCollider.Create(new BoxGeometry { Orientation = quaternion.identity, Size = new float3((sfloat)5.0f, (sfloat)1.0f, (sfloat)50.0f) });
            ...
            else if (m_Counter == 40)
            {
                ...
                for (int i = 0; i < dynamicEntities.Length; i++)
                {
                    ...
                    Assert.IsTrue(translation.Value.y < (sfloat)0.99f, "Box didn't start falling!");
                }
