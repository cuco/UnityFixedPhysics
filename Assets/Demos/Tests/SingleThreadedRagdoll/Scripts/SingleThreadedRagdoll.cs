...
public class SingleThreadedRagdoll : MonoBehaviour
{
    ...
    static readonly System.Type k_DrawComponent = typeof(Fixed.Physics.Authoring.DisplayBodyColliders)
    ...
    public void Update()
    {
        ...
        {
            World = PhysicsWorld,
            TimeStep = (sfloat)World.DefaultGameObjectInjectionWorld.GetExistingSystem<FixedStepSimulationSystemGroup>().Timestep,
    ...
    private struct SingleThreadedPhysicsSimulationJob : IJob
    {
        ...
        {
            NativeArray<Fixed.Physics.Joint> joints = input.World.Joints;
            ...
            for (int i = 0; i < jointInfos.Length; i++)
            {
                ...
                joints[i] = new Fixed.Physics.Joint
                {
                    BodyPair = pair,
                    Entity = Entity.Null,
    ...
    public void Start()
    {
        ...
        var buildPhysicsWorld = World.DefaultGameObjectInjectionWorld.GetExistingSystem<Fixed.Physics.Systems.BuildPhysicsWorld>();
