            ...
            .ForEach((Entity creatorEntity, in CreatePyramids creator) =>
            {
                ...
                for (int p = 0; p < creator.Count; p++)
                {
                    for (int i = 0; i < creator.Height; i++)
                    {
                        ...
                        float3 start = new float3(-(sfloat)rowSize * boxSize.x * (sfloat)0.5f + boxSize.x * (sfloat)0.5f, (sfloat)i * boxSize.y, sfloat.Zero);
                        for (int j = 0; j < rowSize; j++)
                        {
                            float3 shift = new float3((sfloat)j * boxSize.x, sfloat.Zero, (sfloat)p * boxSize.z * (sfloat)creator.Space);
