            ...
            foreach (var jointAuthoringComponent in modifyJointLimits.GetComponents<Component>())
            {
                ...
                foreach (var jointEntity in jointEntities)
                {
                    var angularModification = new ParticleSystem.MinMaxCurve(
                        multiplier: (float)math.radians(modifyJointLimits.AngularRangeScalar.curveMultiplier),
        ...
        Entities
            ...
            .ForEach((ref PhysicsJoint joint, in ModifyJointLimits modification) =>
            {
                var animatedAngularScalar = new FloatRange(
                    (sfloat)modification.AngularRangeScalar.curveMin.Evaluate(time),
                    (sfloat)modification.AngularRangeScalar.curveMax.Evaluate(time)
                );
                var animatedLinearScalar = new FloatRange(
                    (sfloat)modification.LinearRangeScalar.curveMin.Evaluate(time),
                    (sfloat)modification.LinearRangeScalar.curveMax.Evaluate(time)
                );
