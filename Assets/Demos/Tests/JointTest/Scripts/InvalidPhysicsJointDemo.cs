...
public class InvalidPhyiscsJointDemoSystem : SceneCreationSystem<InvalidPhysicsJointDemoScene>
{
    public override void CreateScene(InvalidPhysicsJointDemoScene sceneSettings)
    {
        ...
        // Once the dynamic body is destroyed the joint will be invalid
        {
            ...
            Entity body = CreateDynamicBody(pivotWorld, quaternion.identity, collider, float3.zero, float3.zero, (sfloat)1.0f);

            // create extra dynamic body to trigger Havok sync after the first one is destroyed
            CreateDynamicBody(pivotWorld * (sfloat)2.0f, quaternion.identity, collider, float3.zero, float3.zero, (sfloat)1.0f);
        ...
        {
            // Create a body
            Entity bodyA = CreateDynamicBody(new float3(0, 5.0f, 0), quaternion.identity, collider, float3.zero, float3.zero, (sfloat)1.0f);
            Entity bodyB = CreateDynamicBody(new float3(0, 6.0f, 0), quaternion.identity, collider, float3.zero, float3.zero, (sfloat)1.0f);
