    ...
    where T : SceneCreationSettings
{
    ...
    public Entity CreateBody(float3 position, quaternion orientation, BlobAssetReference<Collider> collider,
        float3 linearVelocity, float3 angularVelocity, sfloat mass, bool isDynamic)
    {
    ...
    {
        return CreateBody(position, orientation, collider, float3.zero, float3.zero, sfloat.Zero, false);
    }

    public Entity CreateDynamicBody(float3 position, quaternion orientation, BlobAssetReference<Collider> collider,
        float3 linearVelocity, float3 angularVelocity, sfloat mass)
    {
...
{
    static readonly Type k_DrawComponent = typeof(Fixed.Physics.Authoring.DisplayBodyColliders)
