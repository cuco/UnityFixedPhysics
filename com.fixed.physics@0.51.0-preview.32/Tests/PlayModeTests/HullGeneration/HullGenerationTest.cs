...
using Material = Fixed.Physics.Material;
...
class RandomConvexGenerator : IPointGenerator
{
    ...
    private int GetNumPoints(ref Fixed.Mathematics.Random rng)
    {
    ...
    public int GetMaxNumPoints(int i)
    {
        Fixed.Mathematics.Random rng = new Fixed.Mathematics.Random(m_Seeds[i]);
...
class RandomEllipsoidGenerator : IPointGenerator
{
    ...
    private void GetResolution(ref Fixed.Mathematics.Random rng, out int res0, out int res1)
    {
...
class HullGenerationTest : MonoBehaviour
{
    ...
    unsafe void CalcHullDistanceStats(ref ConvexHull hull, Fixed.Physics.ConvexCollider* collider, float extraRadius, ref float minDistance, ref float maxDistance)
    {
    ...
    unsafe HullStats TestPointSet(NativeArray<float3> points, float simplificationTolerance, float mergeTolerance, float minAngle, float radius)
    {
        ...
        for (int i = 0; i < simplifiedHull.NumFaces; i++)
        {
            ...
            Fixed.Physics.Plane plane = simplifiedHull.Planes[i];
